...
REGIONYR="HARAMP24"#"ASRAMP23" #"MARAMP22"
# Data Load ---------------------------------------------------------------
Col=read.csv(paste0("./Data/ColonyLevel/",REGIONYR,"_VitalRates_colonylevel_CLEAN.csv"))
Col$TL_Date=ymd(Col$TL_Date)
Col=Col %>% arrange(Site_Genet,TimePt)
...
# Transition Data Out -----------------------------------------------------
write.csv(x = ColonyTransitions,file = paste0("./Data/ColonyTransitions/",REGIONYR,"_ColonyTransitions.csv"))
...
MA=MA %>% select(-X) %>%
  rename(Surface_Area_STA=SArea_STA,Surface_Area_END=SArea_END,
                 l10_Surface_Area_STA=l10_SArea_STA,
                 l10_Surface_Area_END=l10_SArea_END,
                 l10_Surface_Area_ENDann=l10_SArea_ENDann)
AS=AS %>% select(-X) %>%
  rename(Surface_Area_STA=SArea_STA,Surface_Area_END=SArea_END,
                 l10_Surface_Area_STA=l10_SArea_STA,
                 l10_Surface_Area_END=l10_SArea_END,
...
write.csv(x = MAASHA,file = paste0("./Data/ColonyTransitions/MAASHA_22-24_ColonyTransitions.csv"),row.names = FALSE)
dim(MAASHA)
