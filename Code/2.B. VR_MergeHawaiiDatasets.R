...
for( ci in 1:NuCol){
  ...
  if(nrow(thiscol)==1){next()}else{
    #test for SS diff
    SSmatch=data.frame(rES=1:nrow(thiscol),rSS=match(thiscol$EndingDate,thiscol$StartingDate))
    issue=!all(thiscol$EndingSize[SSmatch[,1]]==thiscol$StartingSize[SSmatch[,2]],na.rm=T)
    if(issue){
      issueDF=rbind(issueDF,thiscol)
      Nissue=Nissue+1
    }
  }
  print(paste0("Checked ",ci," colonies. ",Nissue," issues."))
}
print(paste0("Checked ",ci," colonies. ",Nissue," issues."))

#We declare these issues to be annoying and complex, but a small subset of the data (22/1680 ~1% of colonies). We declare them to be annotator error, and
#we hereby drop those colonies with issues!
issuecolid=names(table(issueDF$ColonyID))
ColonyLevel=ColonyLevel %>% filter(!ColonyID%in%issuecolid)
length(unique(ColonyLevel$ColonyID))
...
#Not doing it this ways says Tom: This would roll back to colony level data, instead we're reformatting our existing transition level data below...
#TAO 2026/03/25
# #######################
...
#                             Interval_Years, Mortality, Survival);head(Col19)
# 
# 
...
########################
# ALL WE NEED TO DO HERE is add the appropriate columns, in the right order
# For the TimePt vs TP thing check out the start of 2A
...
# dim(MASHA)
# dim(ColonyLevel)
# tdattesttdata <- ColonyLevel %>% select(c("uniqueID","Site_Genet","Site","Island","Genus_Code","StartingDate","EndingDate",
